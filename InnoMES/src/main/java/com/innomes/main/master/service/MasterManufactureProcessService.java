...
public class MasterManufactureProcessService {
	...
	public Page<MasterManufactureItemDTO> getManufactureItem(MasterProductParam masterProductParam, Pageable pageable) {
		...
		for (MST111 mst111 : content) {
			...
			if (mst111.getMst210().size() > 0) {
				bomList = masterBomService.getBomList(MasterBomParam.builder().prdtId(mst111.getPrdtId()).build());
			}
	...
	public MasterManufactureProcessDTO getManufactureProcess(
		...
		for (MST210 mst210 : content.get().getMst210()) {
			MasterManufactureRoutingDTO routeProcess = MasterManufactureRoutingDTO.builder().prdtId(mst210.getPrdtId())
					.routingRev(mst210.getRoutingRev()).procCode(mst210.getProcCode())
					.procName(masterPoolService.getMST120(mst210.getProcCode()).getProcName())
					.inOutType(masterPoolService.getMST120(mst210.getProcCode()).getInOutType())
					.routingSeq(mst210.getRoutingSeq()).procSeq(mst210.getProcSeq()).inQnt(mst210.getInQnt())
					.outQnt(mst210.getOutQnt()).qntUnit(mst210.getQntUnit()).leadTime(mst210.getLeadTime())
					.settingTime(mst210.getSettingTime()).unitWeight(mst210.getUnitWeight())
					.locCode(mst210.getLocCode()).build();

			List<MasterBomDTO> bomList = masterBomService.getBomList(
					MasterBomParam.builder().prdtId(mst210.getPrdtId()).procCode(mst210.getProcCode()).build());
	...
	public boolean saveManufactureProcess(MasterManufactureProcessParam manufactureProcessParam) {
		...
		try {
			...
			if (maxRev == null) {
				...
				MST200PK delMst200 = MST200PK.builder().prdtId(manufactureProcessParam.getPrdtId()).routingRev(maxRev)
						.build();
			...
			for (int i = 0; i < manufactureProcessParam.getRouteList().size(); i++) {
				MST210 mst210 = MST210.builder()
						.prdtId(manufactureProcessParam.getPrdtId()).routingRev(maxRev)
	...
	// 제조공정정보 수정
	public boolean updateManufactureProcess(MasterManufactureProcessParam manufactureProcessParam) {
		boolean success = true;

		try {
			Integer maxRev = mst200Repository.getMaxRoutingRev(manufactureProcessParam.getPrdtId());
			
			MST200 mst200 = mst200Repository.findById(MST200PK.builder()
					.prdtId(manufactureProcessParam.getPrdtId())
					.routingRev(maxRev)
					.build()).orElseThrow(CManufactureProcessNotFoundException::new);
			
			mst200.setDescription(manufactureProcessParam.getDescription());
			mst200.setPrdtStatus(manufactureProcessParam.getPrdtStatus());
			mst200.setUpdateUser(manufactureProcessParam.getUpdateUser());
			mst200.setUpdateTime(new Date());
			mst200.setNew(false);
